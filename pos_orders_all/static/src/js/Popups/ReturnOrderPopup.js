	...
	class ReturnOrderPopup extends AbstractAwaitablePopup {
		...
		do_returnOrder(){
			...
			$.each(list_of_qty, function(index, value) {
                ...
                let product_name = $(value).closest('tr').find('td:first-child').text();
                let entered_item_notes = $(value).closest('tr').find('.entered_item_notes input[name="notes"]');
                let returned_notes = entered_item_notes.val();
				...
				if (!isNaN(entered_qty) && entered_qty !== 0){
                    let params = {
                                    ...
                                    scrap_qty: entered_qty,
                                    origin: returned_notes,
                                };
                                rpc.query({
                                    model: 'stock.scrap',
                                    method: 'action_scrap_order',
                                    args: [params],
                                });
                }
