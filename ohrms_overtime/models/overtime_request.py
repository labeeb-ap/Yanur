...
class HrOverTime(models.Model):
    ...
    overtime_amount_per_hrs = fields.Float(string='Overtime Amount per Hours',default=2.50)
    overtime_amount_total = fields.Float(string='Total Overtime Amount')
    ...
    def approve(self):
        if self.overtime_amount_total:
            self.cash_hrs_amount = self.overtime_amount_total
    ...
    @api.onchange('date_from', 'date_to', 'employee_id', 'overtime_amount_per_hrs')
    def _onchange_date(self):
        if self.employee_id and self.date_from and self.date_to:
            ...
            if contract:
                ...
                # amt = 206.19 * ((contract.wage / month_range) / 8)
                amts = self.overtime_amount_per_hrs * self.days_no_tmp
                # self.overtime_amount_per_hrs = amt
                self.overtime_amount_total = amts
            else:
                self.overtime_amount_per_hrs = 0
                self.overtime_amount_total = 0
