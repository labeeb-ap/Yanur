...
class HrLoan(models.Model):
    ...
    def action_approve(self):
        for data in self:
            ...
            if not data.debit_account_id or not data.credit_account_id or not data.journal_id:
                raise ValidationError(_("Please Set Debit,Credit account and Journal"))
            else:
            ...
            move_id.write({'line_ids': [(0, 0,
                                         {'account_id': data.credit_account_id.id,
                                          'partner_id': data.employee_id.user_id.partner_id.id,
                                          'name': 'Salary Loan:' + data.employee_id.name,
                                          'debit': 0, 'credit': data.loan_amount, }),
                                        (0, 0,
                                         {'account_id': data.debit_account_id.id,
                                          'partner_id': data.employee_id.user_id.partner_id.id,
                                          'name': 'Salary Loan:' + data.employee_id.name,
                                          'debit': data.loan_amount, 'credit': 0, })
                                        ]})
            move_id.action_post()
